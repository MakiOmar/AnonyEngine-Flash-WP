...
class Anony_Flash_Wp_Public {
	...
	public function wp_html_compression_finish_cb($html)
	{
	    ...
	    if($anofl_options->compress_html != 1) { return $html;
	    }
    ...
    public function defer_scripts ( $tag, $handle, $src ) {
        $anofl_options = ANONY_Options_Model::get_instance('Anofl_Options');
        if (is_admin() || $anofl_options->defer_scripts !== '1' ) { return $tag; //don't break WP Admin
        }
    ...
    public function disable_wp_embeds()
    {
        ...
        if($anofl_options->disable_embeds == '1' ) { $keep = false;
        }
        
        if($anofl_options->enable_singular_embeds == '1' && is_single()) { $keep = true;
        }
	...
	public function load_scripts_on_wc_templates_only()
	{
	    ...
	    if ($anofl_options->wc_shop_only_scripts != 1) { return;
	    }
	...
	public function disable_product_mobile_srcset ( $image_meta ) 
	{
		if ( class_exists( 'woocommerce' ) && !is_single() && wp_is_mobile() )
		{
			...
			if ( '1' == $anofl_options->wc_disable_srcset  ) return false ;
	...
	public function product_custom_mobile_thumb_size ( $size ) 
	{
		...
		if ( empty( $anofl_options->wc_mobile_thumb_size ) ) return $size ;
	...
	public function dequeued_styles () {
        ...
        $dequeued_styles = apply_filters( 'anony_dequeue_styles', [
            'wpml-tm-admin-bar',
            'wc-blocks-vendors-style',
            'wc-blocks-style-rtl',
            'wp-block-library',
    ...
    public function dequeue_scripts(){
    	if ( is_admin() ) return;
        $dequeued_scripts = apply_filters( 'anony_dequeue_scripts', [
            'allow-webp-image',
            'jupiterx-child',
            'jupiterx-utils'
        ] );
